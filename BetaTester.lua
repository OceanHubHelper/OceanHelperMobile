...
--------------------------------------------------
-- Hover Button (Paste-at-bottom Add-on)
--------------------------------------------------
do
	-- make sure makeBtn and layout vars exist
	if typeof(makeBtn) ~= "function" then
		warn("Hover add-on: makeBtn not found.")
		return
	end

	local HCard, H = makeBtn("Hover", WIDE_W, SIZE)

	-- place under existing stack (no need to modify setPositions)
	local function placeHover()
		if isPhone then
			local cx = screen.X - WIDE_W - 12
			local cy = (screen.Y * 0.55) - 110
			HCard.Position = UDim2.fromOffset(cx, cy + (SIZE + GAP) * 2)
		else
			local bx = screen.X - 360
			local by = screen.Y - 360
			HCard.Position = UDim2.fromOffset(bx, by + (SIZE + GAP) * 2)
		end
	end
	placeHover()

	-- Hover logic (Y-axis lock)
	...
	player.CharacterAdded:Connect(function(char)
		...
		H.BackgroundColor3 = Color3.fromRGB(28,30,40)
	end)
	...
	local function enableHover()
		...
		hoverConn = RunService.Heartbeat:Connect(function()
			if not hoverEnabled or not hrp or not hrp.Parent then return end
	...
	H.MouseButton1Click:Connect(function()
		...
		if hoverEnabled then
			H.BackgroundColor3 = Color3.fromRGB(60,120,80) -- ON
			enableHover()
		else
			H.BackgroundColor3 = Color3.fromRGB(28,30,40) -- OFF
			disableHover()
		end
	end)
end
